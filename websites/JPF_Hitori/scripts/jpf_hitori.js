...
/*
   ...
   */
   ...
   function startUp() {
      ...
      // Event handler to check the puzzle if it is finished
      document.getElementById("check").addEventListener("click", findErrors);
      // Event handler to show the solution
   ...
   */
   function setupPuzzle() {
      ...
      // Set the initial color to be white
      for (var i = 0; i < allCells.length; i++) {
         ...
         // Event Listener that listens for event down
         document.addEventListener("mouseover",
            function(e) {
               if (e.shiftKey) {
                  ...
                  allCells[i].style.cursor = "url(images/jpf_pencil.png), pointer";
               }
            }
         );

         // Event listener for the mouse up event
   ...
   */
   function switchPuzzle(e) {
      if (confirm("Are you sure? You will lose all of your work on the puzzle.")) {
         var puzzleId = e.target.id;
         // Changing the puzzle title
   ...
   */
   function findErrors() {
      ...
      setTimeout(function() {
         ...
         for (var i = 0; i < allCells.length; i++)
            if (allCellsCells[i].style.backgroundColor === "red")
               allCells[i].style.backgroundColor = "white";
      }, 100);
   }
